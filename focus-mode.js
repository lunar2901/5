...
export function initFocusMode({
  ...
  const keyVocabCollapsed = `${storageKey}:vocabCollapsed:${level}`; // "1" | "0"
  ...
  // Collapsible vocabulary panel (default: collapsed on small screens)
  const defaultCollapsed = typeof window !== "undefined" && window.matchMedia
    ? window.matchMedia("(max-width: 768px)").matches
    : false;
  let vocabCollapsed = (localStorage.getItem(keyVocabCollapsed) ?? (defaultCollapsed ? "1" : "0")) === "1";
  ...
  function save() {
    ...
    localStorage.setItem(keyVocabCollapsed, vocabCollapsed ? "1" : "0");
  }
  ...
  function toggleVocabCollapsed() {
    vocabCollapsed = !vocabCollapsed;
    // If collapsing, also close inner panels to keep UI calm.
    if (vocabCollapsed) {
      openPanel = "";
      hideWords = true;
    }
  ...
  function render() {
    ...
    const currentLearned = isLearned(currentItem, index);
	    ...
	    root.innerHTML = `
	      <section class="vocab-panel ${vocabCollapsed ? "collapsed" : ""}">
	        <button type="button" class="vocab-row vocab-top" data-action="toggleVocab">
	          <span>Vocabulary</span>
	          <span class="chev">${vocabCollapsed ? "▼" : "▲"}</span>
	        </button>

	        <div class="vocab-inner" ${vocabCollapsed ? "hidden" : ""}>
    ...
    root.querySelectorAll("[data-action]").forEach((el) => {
      el.addEventListener("click", () => {
        const a = el.getAttribute("data-action");
	        if (a === "toggleVocab") toggleVocabCollapsed();
    ...
    window.onkeydown = (e) => {
      ...
      if (e.key === "ArrowRight") next();
	      if (e.key.toLowerCase() === "v") toggleVocabCollapsed();
